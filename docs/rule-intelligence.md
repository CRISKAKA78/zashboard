...
响应包含 `provider`、`total`、`matched`、`page`、`pageSize`、`hasMore`、四类 `counts`、`items` 和 `cache`。`default` 严格保持 Provider 原始顺序；相同排序值也以原始序号稳定排序。
...
| `MIHOMO_RULES_DIR`                          | `<源配置目录>/rules`      | 允许读取的 Provider 根目录           |
...
- `text` 与 `yaml` Provider 直接归一化为 `{type, value, raw}`。
