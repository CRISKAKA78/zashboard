...
// See src/assets/font-loaders/ for what each value loads.
const font = process.env.FONT || 'all'
const supportedFonts = new Set(['all', 'cdn', 'firasans', 'misans', 'pingfang', 'sarasa', 'none'])
const selectedFont = supportedFonts.has(font) ? font : 'all'
...
export default defineConfig({
  ...
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        ...
        globPatterns: ['**/*.{js,css,html,ico,png,svg,woff2,webp,jpg,md,txt}'],
  ...
  resolve: {
    alias: {
      ...
      'zashboard-font-loader': fileURLToPath(
        new URL(`./src/assets/font-loaders/${selectedFont}.ts`, import.meta.url),
      ),
    },
  },
})
