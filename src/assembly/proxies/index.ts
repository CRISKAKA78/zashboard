...
import { resolveProxyChain } from '@/features/rule-intelligence/proxyChain'
...
export const getProxyChainAnalysis = (name: string) =>
  resolveProxyChain(name, proxyMap.value, new Set(proxyGroupList.value))

export const getNowProxyNodeName = (name: string) =>
  getProxyChainAnalysis(name).finalOutbound || name

export const getProxyGroupChains = (name: string) => {
  if (!proxyMap.value[name]) {
    return []
  }

  const groupNames = new Set(proxyGroupList.value)
  const result: string[] = []

  for (const item of getProxyChainAnalysis(name).path) {
    if ((item === name || groupNames.has(item)) && !result.includes(item)) {
      result.push(item)
    }
  }
