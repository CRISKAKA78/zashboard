<template>
  <div
    class="hover:bg-base-200/40 cursor-pointer px-4 py-2.5 sm:flex sm:items-center sm:gap-4"
    role="button"
    tabindex="0"
    @click="$emit('open', ruleProvider)"
    @keydown.enter.self="$emit('open', ruleProvider)"
    @keydown.space.self.prevent="$emit('open', ruleProvider)"
  >
...
defineEmits<{
  (e: 'open', provider: RuleProvider): void
}>()
